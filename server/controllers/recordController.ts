...
export const createRecord = async (record: PostRecord): Promise<Record> => {
  const results = await sql`
    INSERT INTO Expedientes (id_estudiante, ano_inicio, ano_fin, id_ciclo, turno, convocatoria, fecha_pago_titulo)
    VALUES (${record.id_estudiante}, ${record.ano_inicio}, ${record.ano_fin}, ${record.id_ciclo}, ${record.turno}, ${record.convocatoria}, ${record.fecha_pago_titulo ?? null})
    RETURNING *
  `;
